...
In addition you will need to turn off the "legacy way" of handling Flink and Spark runtimes I the cloudflow operator, you can do that by upgrading the Helm installation and adding two additional Java properties. e.g. `-Dcloudflow.platform.flink-enabled=false -Dcloudflow.platform.spark-enabled=false`.
...
NOTE: the NFS Server Provisioner is an excellent and easy to setup storage in development environemt, for production use the suggested and supported Cloud integrations for https://ci.apache.org/projects/flink/flink-docs-master/docs/deployment/filesystems/overview/#pluggable-file-systems[Flink] and for https://spark.apache.org/docs/3.1.1/cloud-integration.html#important-cloud-object-stores-are-not-real-filesystems[Spark]
