...
trait ItSetup {
  ...
  def safeCleanup(namespace: String) = {
    ...
    Try { cli.exec("flink/undeploy", "./undeploy-application.sh swiss-knife") }
    Try { cli.exec("spark-cli/undeploy", "./undeploy-application.sh swiss-knife") }
    withK8s { k8s =>
  ...
  def withMatchingStreamletPod[A](status: ApplicationStatus, streamletName: String, partialPodName: String)(
    ...
    withK8s { k8s =>
      val podName = k8s
        .pods()
        .inNamespace(status.summary.namespace)
        .list()
        .getItems()
        .asScala
        .find { p =>
          val name = p.getMetadata.getName
          name.contains(streamletName) && name.contains(partialPodName)
        }
        .get
        .getMetadata
        .getName

      action(k8s.pods().inNamespace(status.summary.namespace).withName(podName))
    }
  }
